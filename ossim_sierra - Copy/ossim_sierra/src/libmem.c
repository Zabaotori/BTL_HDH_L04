 ...
 */
 ...
 int pg_getpage(struct mm_struct *mm, int pgn, int *fpn, struct pcb_t *caller)
 {
   ...
   { /* Page is not online, make it actively living */
     ...
     __swap_cp_page(caller->active_mswp, PAGING_PTE_SWP(pte), caller->mram, *fpn);
     ...
     */
     ...
     //pte_set_invalid(&mm->pgd[pgn]);
     mm->pgd[vicpgn] |= PAGING_PTE_SWAPPED_MASK;
     mm->pgd[vicpgn] &= ~PAGING_PTE_PRESENT_MASK;
     SETVAL(mm->pgd[vicpgn], swpfpn, PAGING_PTE_SWPOFF_MASK, PAGING_PTE_SWPOFF_LOBIT);
     ...
     mm->pgd[vicpgn] &= ~PAGING_PTE_SWAPPED_MASK;
     mm->pgd[vicpgn] |= PAGING_PTE_PRESENT_MASK;
     SETVAL(mm->pgd[vicpgn], swpfpn, PAGING_PTE_FPN_MASK, PAGING_PTE_FPN_LOBIT);
