...
int __sys_killall(struct pcb_t *caller, struct sc_regs* regs) {
     ...
     */
    struct pcb_t *k_process [MAX_PRIO]; // this list to add processed required to kill
    int index = 0;
    ...
    if (index > 0){
        ...
        for(int i=0;i<index;i++) {
            ...
            if (k_process[i]->code->text[j].opcode==ALLOC) {  //Free all allocated region of killed process
               __free(k_process[i],k_process[i]->mm->mmap->vm_id, k_process[i]->code->text[j].arg_0);
            }
        }
    }
    else
        printf("Process with name %s is not exist\n",proc_name);
